		...
		find: function(context, options, callback) {
			...
			for ( property in ctx ) {
				if ( ctx.hasOwnProperty(property) ) {
					callback(ctx[property], (( options.delimiter && ctx[property][options.property] ) ? ( ctx[property][options.property].split(options.delimiter).indexOf(options.value) > -1 ) : ( ctx[property][options.property] === options.value )));
				}
			};
		},
